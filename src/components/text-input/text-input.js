...
import withKeyHandler from '../KeyHandler';
...
class TextInput extends React.Component {
  ...
  onArrowDown(evt, suggestions) {
    ...
    const newSelection = selectedSuggestion + 1 > suggestions.length - 1
      ? -1 : selectedSuggestion + 1;
  ...
  onArrowUp(evt, suggestions) {
    ...
    const newSelection = selectedSuggestion === -1
      ? suggestions.length - 1 : selectedSuggestion - 1;
  ...
  render() {
    ...
    const filteredSuggestions = !value ? [] : suggestions.filter(s =>
    ...
    return (
      <div className={clsNs('text-input-wrapper')}>
        <KeyHandlingInput {...{
          ...
          onBlur: this.onBlur,
          onArrowDown: showSuggestions ? (evt => this.onArrowDown(evt, filteredSuggestions)) : null,
          onArrowUp: showSuggestions ? (evt => this.onArrowUp(evt, filteredSuggestions)) : null,
          onEnter: showSuggestions ? (evt => this.onEnter(evt, filteredSuggestions)) : null,
          onTab: showSuggestions ? (evt => this.onTab(evt, filteredSuggestions)) : null,
          onEscape: showSuggestions ? (evt => this.onEscape(evt, filteredSuggestions)) : null
        }}/>
        {showSuggestions &&
          <ul {...{
            ...
            {filteredSuggestions.map((s, i) => <li {...{
              className: clsNs(
                'text-input-suggestion',
                i === selectedSuggestion && 'selected'),
              role: 'option',
              key: i
            }}><span className="match">{s.substring(0, value.length)}</span>{s.substring(value.length)}</li>)}
          </ul>
        }
      </div>
    );
