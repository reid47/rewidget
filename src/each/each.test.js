  ...
  describe('when given data items', () => {
    ...
    describe('when given a children function', () => {
      ...
      it('calls the children function to render each data item', () => {
        expect(root.find('.item').length).toBe(3);
        expect(root.find('.item').at(0).hasClass('item-one-0')).toBe(true);
        expect(root.find('.item').at(1).hasClass('item-two-1')).toBe(true);
        expect(root.find('.item').at(2).hasClass('item-three-2')).toBe(true);
      });
      ...
      describe('when given a filter', () => {
        ...
        it('filters the children before rendering', () => {
          expect(root.find('.item').length).toBe(2);
          expect(root.find('.item').at(0).hasClass('item-one-0')).toBe(true);
          expect(root.find('.item').at(1).hasClass('item-three-1')).toBe(true);
        });
      });
      ...
      describe('when given a postFilter', () => {
        beforeEach(() => {
          root.setProps({
            postFilter: child => child.props.className.indexOf('item-three-2') !== -1
          });
        });

        it('filters the children after rendering', () => {
          expect(root.find('.item').length).toBe(1);
          expect(root.find('.item').at(0).hasClass('item-three-2')).toBe(true);
        });
      });
    });
  });
